...
def clean_class_cond_fn(x_t, y, classifier,
    ...
    if use_logits:
        ...
        #selected = -(F.logsigmoid(selected)*weight)
    ...
    def __call__(self, imgs, cfs, noises, target, label,
        ...
        for idx in range(len(imgs)):
            ...
            self.save_img(img=imgs[idx],
                          ...
                          noise=noises[idx],
                          idx=current_idx,
                          target=str(target[idx].cpu().numpy()),
                          label=str(label[idx].cpu().numpy()),
                          pred=str(pred[idx].cpu().numpy()),
                          pred_cf=str(pred_cf[idx].cpu().numpy()),
                          bkl="["+", ".join([f"{l:.4f}" for l in bkl[idx]])+"]",
                          l_1=l_1[idx].item(),
                          mask=mask)
...
class SlowSingleLabel():
    def __init__(self, dataset, maxlen=float('inf')):
        self.dataset = dataset
        self.indexes = []
